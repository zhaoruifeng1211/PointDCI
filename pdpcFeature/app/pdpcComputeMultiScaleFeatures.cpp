...
int main(int argc, char **argv)
{
    ...
    GeometricVariation geometric_variation(point_count, scale_count);
    std::vector<Scalar> iterRadius;
    // sampling stuff
...
//            debug() << sampling.size() << " points sampled";
        ...
        // 2.2 Features --------------------------------------------------------
        {
            ...
            for(int i=0; i<point_count; ++i)
            {
                ...
                if(mls.stable())
                {
                    ...
                    geometric_variation.m_geometric_variation_n(i,j) = mls.fit().normal();
                    geometric_variation.m_geometric_variation_uc(i,j) = mls.fit().potential();
                    geometric_variation.m_geometric_variation_uq(i,j) = (mls.fit().k1()+mls.fit().k2())*scale/2.0;
                ...
                else
                {
                    ...
                    geometric_variation.m_geometric_variation_n(i,j) = Vector3::Zero();
                    geometric_variation.m_geometric_variation_uc(i,j) = 0;
                    geometric_variation.m_geometric_variation_uq(i,j) = 0;

                }
            } // for i
        }
        {
            PointCloud savedCloud;
            for (const auto& p:iterPoints){
                savedCloud.emplace_back(p);
            }
            Loader::Save(in_output+"_"+std::to_string(j)+"scaleProj.ply", savedCloud, true);
        }
    ...
    geometric_variation.save(in_output + "_geometric_variation.txt");
    {
        const std::string iterRadiusFilename = in_output + "_iterRadius.txt";
        //save iterRadius
        std::ofstream ofs(iterRadiusFilename);
        if(!ofs.is_open())
        {
            error().iff(true) << "Failed to open output scales file " << iterRadiusFilename;
            return false;
        }
    
        for(const Scalar s : iterRadius)
        {
            ofs << s << " ";
        }
    
        info().iff(true) << iterRadius.size() << " scales saved to " << iterRadiusFilename;
    ...
    // Append runtime to the local log file
    std::ofstream log(in_output+"runtime_log.txt", std::ios::app);
    if (log.is_open()) {
        log << "Program runtime: " << minutes << " minutes\n";
        log.close();
    } else {
        std::cerr << "Failed to open the runtime log file." << std::endl;
    }
    return 0;
}
