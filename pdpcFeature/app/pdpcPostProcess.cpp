...
int main(int argc, char **argv)
{
    ...
    if(!in_ranges.empty())
    {
        if(in_ranges.size() % 2 == 0)
        {
            for(int n=0; n<int(in_ranges.size())/2; ++n)
            {
                ...
                // save
                const std::string filename = in_output + "_Range" + str::to_string(pers_min,3) + "_" + str::to_string(pers_max,3);
    ...
    if(!in_pers.empty())
    {
        for(const auto& str : in_pers)
        {
            ...
            std::vector<ComponentData> smallerComp;
            ...
            for(int c=0; c<comp_data.size(); ++c)
            {
                ...
                {
                    smallerComp.push_back(comp_data[c]);
                    // comp are sorted by pers
                    // break;
                }
            }
            auto labeling_tmp=labeling;
            for(int i=0;i<labeling_tmp.size();i++){
                if(labeling_tmp[i]==-1){
                    reLabeling[i]=-2;
                }
            }
            for(auto it = smallerComp.rbegin(); it != smallerComp.rend(); ++it){
                cCheck++;
                for(auto i:it->indices()){
                    if(reLabeling.find(i)!=reLabeling.end()){
                        reLabeling[i]=cCheck;
                    }
                }
            }
            for(auto it = reLabeling.begin(); it != reLabeling.end(); ++it){
                labeling_tmp[it->first]=it->second;
            }

            // Save the original labeling
            const std::string filename_label = in_output + "_PersLabel" + str::to_string(pers,3);
            std::ofstream ofs(filename_label+".txt");
            if (!ofs.is_open()) {
                std::cerr << "Failed to open output file." << std::endl;
                return -1;
            }

            // Write point index and label, one pair per line
            int idx=0;
            for (const auto& kv : labeling) {
                ofs << idx << " " << kv << "\n";
                idx++;
            }

            ofs.close();
            std::cout << "Saved labeling file." << std::endl;
            // Save the completed relabeling
            const std::string filename_relabel = in_output + "_PersReLabel" + str::to_string(pers,3);
            std::ofstream ofs_re(filename_relabel+".txt");
            if (!ofs_re.is_open()) {
                std::cerr << "Failed to open output file." << std::endl;
                return -1;
            }

            // Write point index and relabel, one pair per line
            for (const auto& kv : reLabeling) {
                ofs_re << kv.first << " " << kv.second << "\n";
            }

            ofs_re.close();
            std::cout << "Saved relabeling file." << std::endl;
            Segmentation seg(labeling);
            seg.invalidate_small_region(10);

            Segmentation seg_tmp(labeling_tmp);
            seg_tmp.invalidate_small_region(10);

            // save
            const std::string filename = in_output + "_Pers" + str::to_string(pers,3);
            ...
            if(in_col)
            {
                ...
                Loader::Save(filename + ".ply", points, in_v);
                Loader::saveVGFast(filename+".vg",points,labeling,200,true);
            }

            if(1){
                            // save
                const std::string filename_tmp = in_output + "_FullSegPers" + str::to_string(pers,3);
                seg_tmp.save(filename + ".txt");
                if(in_col)
                {
                    points.request_colors();
                    const auto colormap = Colormap::Tab20();
                    seg_tmp.set_colors(points.colors_data(), Colors::Black(), colormap);
                    Loader::Save(filename_tmp + ".ply", points, in_v);
                    Loader::saveVGFast(filename_tmp+".vg",points,labeling_tmp,200,true);
                }
            }
        }
    }
    ...
    if(!in_scales.empty())
    {
        for(std::string str : in_scales)
        {
            ...
            // save
            const std::string filename = in_output + "_Scale" + str::to_string(idx_scale,3);
            seg.save(filename + ".txt");
            if(in_col)
            {
                points.request_colors();
                ...
                seg.set_colors(points.colors_data(), Colors::Black(), colormap);
                Loader::Save(filename + ".ply", points, in_v);
                Loader::saveVG(filename+".vg",points,labeling,200,true);
            }
        }
    }
