...
GeometricVariation::GeometricVariation(int point_count, int scale_count) :
    m_point_count(point_count),
    m_scale_count(scale_count)
{
    geometric_variation_n.resize(point_count * scale_count);
    geometric_variation_uc.resize(point_count * scale_count);
    geometric_variation_uq.resize(point_count * scale_count);
}
bool GeometricVariation::save(const std::string& filename){
    const std::string ext = filename.substr(filename.find_last_of(".") + 1);
    if(ext == "txt")
    {
        std::ofstream ofs(filename);
        if(!ofs.is_open())
        {
            error().iff(true) << "Failed to open output features file " << filename;
            return false;
        }
        ...
        for(int i=0; i<m_point_count; ++i)
        {
            for(int j=0; j<m_scale_count; ++j)
            {
                ofs << m_geometric_variation_uc(i,j)   <<" "
                    << m_geometric_variation_n(i,j)[0] << " "
                    << m_geometric_variation_n(i,j)[1] << " "
                    << m_geometric_variation_n(i,j)[2] << " "
                    << m_geometric_variation_uq(i,j)   <<" ";
            }
            ofs << "\n";
        }
        return true;
